...
4. The text is typed into whatever has keyboard focus on the connected PC, followed by a trailing newline (Enter) so the cursor drops to the next line/row once the whole scan is typed out, ready for the next one. If the scanned text itself has multiple lines, each line break also types as a new line (Enter) rather than being joined with a space, so a multi-line label/form reads the same way on the PC as it did on the scan. At the same time, the photo is saved into the phone's Gallery (in its own "AI Scan" album) with the approved text embedded as EXIF metadata (`ImageDescription`/`UserComment`) **and** as a plain `.txt` file saved right alongside the photo in that same album folder - this happens regardless of whether the PC send succeeded, since the scan is still worth archiving even if the PC wasn't reachable at that moment.
...
- The extracted text is embedded in each saved photo's EXIF metadata, but whether your phone's Gallery app *displays* it in its "Details"/"Info" panel depends on that app - **Google Photos, the default Gallery on most Android/GMS devices, does not show it at all**, even though it's genuinely written to the file. Because of that, the same text is also saved as a plain `.txt` file with a matching name right next to the photo, in the same "Pictures/AI Scan" folder.
- For devices with no Gallery/Photos app at all (common on locked-down enterprise configurations of the EDA51), the app has its own **History** screen (tap "History" from the capture screen) that lists every past scan with its thumbnail and full text, plus a calendar view to jump to scans from a specific day - all read directly from the same on-device "Pictures/AI Scan" folder, with no separate database or external app required.

## Verification status

**Confirmed working end-to-end** on a real Honeywell EDA51 paired with a Windows laptop: camera capture, on-device OCR with confidence display, review/edit/approve, Bluetooth RFCOMM connection, and keystroke injection into Notepad via the Windows companion program. Two real-world quirks worth knowing if you hit them again on a different PC:

- `createRfcommSocketToServiceRecord()` (the "standard" way to connect) failed with a known Android bug (`read failed, socket might closed or timeout, read ret: -1`) on the tested hardware/PC combination - `BluetoothSppManager` automatically falls back to connecting on a fixed RFCOMM channel (1-30) when that happens, which is what actually worked in testing.
- The Windows companion program must be *running* for anything to happen, and keystrokes go to whatever window currently has OS focus - click into the target app (Excel, Notepad, etc.) before tapping Approve & Send, the same as with a physical keyboard.

This project was authored in an isolated cloud sandbox whose network policy blocks Google's Android artifact host (`dl.google.com`), so the Android app itself could not be compiled from within that sandbox - it was written and carefully reviewed there, then fixed against real compile errors once built on an actual developer machine (see git history for the specific fixes that came out of that process). The Windows companion program was fully compiled and verified in that same sandbox (`dotnet build`, 0 errors).
